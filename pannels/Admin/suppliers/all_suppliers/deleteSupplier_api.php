    ...
    if($conn){
        if(isset($_SESSION['userId'])){//if the user id session value is set then go ahead to make chances 
            ...
            $prev_product_id=$row['product_id'];
                ...
                $results = $conn->query($sql);//delete the supplier
    
                if ($results) {//if supplier is updated then log the information
    
                    $sql_for_logs="INSERT INTO logs(user_id_for_logs,action_type,described,new_values,ip_address,affected_table,affected_record_id) VALUES($userId,'delete','supplier_id=>$supplierId,fname=>$prev_fname::lname=>$prev_lname::prev_s_contact=>$prev_s_contact::prev_s_location=>$prev_s_location::prev_product_id=>$prev_product_id','empty','$ipAddress','supplier','$supplierId')";
                    ...
                    echo "<div style='background-color: #C6F7D0; padding: 20px; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);'>
                            ...
                            <h2 style='font-weight: bold; margin-top: 10px;'>Supplier info has been deleted successfully!</h2>
                            <p style='margin-bottom: 20px;'>Supplier info has been deleted successfully.</p>
                        </div>";
                } else {
