...
<!-- LOADER  END-->
        <div class="search">
            <input type="text" placeholder="Search Profit by date">
...
<!-- 
  ...
  <tbody>
    ...
    try {
        ...
        $sql = "SELECT date_of_sale, product_Name, SUM(profit) AS totalProfit FROM sales GROUP BY date_of_sale, product_Name ORDER BY date_of_sale desc;";//fetch all  total date profits from the  sales table
        ...
        if ($results) { // Check if the query was successful and returned a mysqli_result object
            if (mysqli_num_rows($results) > 0) { // Check if there are any rows in the products table
                ...
                while ($row = mysqli_fetch_assoc($results)) {
                    echo "
                            <tr class='tbRow'>
                            <th scope='row' class='tbData'>{$row['date_of_sale']}</th>
